      ...
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <section className="bg-white p-6 rounded-xl shadow-lg">
          ...
          <form onSubmit={handleUpdateProfile} className="space-y-4">
            <div>
              ...
              <input type="text" value={user?.email} disabled className="w-full p-3 bg-gray-200 border border-gray-300 rounded-lg cursor-not-allowed text-gray-500" />
            </div>
            <div>
              <label htmlFor="fullName" className="block mb-1 font-medium text-gray-700">Nombre Completo</label>
              <input id="fullName" type="text" value={fullName} onChange={(e) => setFullName(e.target.value)} className="w-full p-3 border border-gray-300 rounded-lg text-gray-900" />
            </div>
            <div>
              <label htmlFor="specialty" className="block mb-1 font-medium text-gray-700">Especialidad</label>
              <input id="specialty" type="text" value={specialty} onChange={(e) => setSpecialty(e.target.value)} className="w-full p-3 border border-gray-300 rounded-lg text-gray-900" />
            </div>
            <div>
              <label htmlFor="bio" className="block mb-1 font-medium text-gray-700">Biografía Corta</label>
              <textarea id="bio" value={bio} onChange={(e) => setBio(e.target.value)} className="w-full p-3 border border-gray-300 rounded-lg min-h-[120px] text-gray-900" />
            </div>
        ...
        <section className="bg-white p-6 rounded-xl shadow-lg">
           ...
           <p className="mb-4 text-gray-700">
              {locationSaved ? '✅ Tu ubicación está guardada.' : '❌ Aún no has guardado tu ubicación para que los clientes te encuentren.'}
            </p>
        ...
        <section className="bg-white p-6 rounded-xl shadow-lg">
          ...
          <div className="mt-6">
            ...
            <input id="workImage" type="file" accept="image/*" onChange={(e) => e.target.files && setSelectedFile(e.target.files[0])} disabled={uploading} className="block text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-red-50 file:text-red-700 hover:file:bg-red-100" />
            <Button type="button" onClick={handleImageUpload} disabled={uploading || !selectedFile} className="mt-4 bg-purple-600 hover:bg-purple-700 focus:ring-purple-500 text-white">
